...
class Board {
  ...
  attemptChordOrDig(
    ...
    if (!this.checkCoordsInBounds(tileX, tileY)) {
      //Click not on board, exit (doesn't count as wasted click)
      this.boardInput.updateDepressedSquares(tileX, tileY, false, touchIdentifier); //Undepress square as we have just done leftMouseUp
      return;
    }
  ...
  attemptFlagOrChord(unflooredTileX, unflooredTileY, touchIdentifier) {
    ...
    this.boardInput.updateDepressedSquares(tileX, tileY, false, touchIdentifier);
  ...
  doLose() {
    ...
    this.boardInput.clearAllDepressedSquares();
    this.boardRenderer.clearTimerTimeout();
