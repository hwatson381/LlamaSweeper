  ...
  */
  ...
  copyBoardLink() {
    ...
    if (variant.value === "zini explorer") {
      ...
      let hrefWithClickString = this.router.resolve({
        name: "play",
        params: { variant: urlVariant },
        query: query,
      }).href;
  ...
  async getScreenshotBlob(includeStats) {
    ...
    if (!includeStats) {
      const blob = await toBlob(this.mainCanvas.value); //Using html-to-image here as this.mainCanvas.value.toBlob() would miss css filters applied to canvas.
      ...
      const boardCanvas = await toCanvas(this.mainCanvas.value); //Using html-to-image here as this.mainCanvas.value would miss css filters applied to canvas.
